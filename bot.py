...
class ScheduleReporter(discord.Client):
    ...
    async def report_week_tier(self, tier, week, newMessage=False):
        fields = []
        ...
        footer = {'text': 'Generated by fwScheduleBot - Last Updated: {}'.format(datetime.now())}
    ...
    async def report_today_event(self, newMessage=False):
        ...
        for event in current_events:
            ...
            if event_date == datetime.today().strftime("%x"): 
                ...
                fields.append({"name": "{} VS {}".format(team1name, team2name), "value": value})

        if len(fields) == 0:
            fields.append({"name": "No games scheduled for today.", "value": "No games scheduled for today."})
        footer = {'text': 'Generated by fwScheduleBot - Last Updated: {}'.format(datetime.now())}
        data = {"title": "Today's Scheduled Games: {}".format(datetime.today().strftime("%A, %B %d")), "fields": fields, "footer": footer,}
        embed = discord.Embed.from_dict(data)
        message_configuration = 'TodayEventMessageId'
        channel = client.get_channel(config['DailyScheduleChannel'])
        if message_configuration in config: 
            message = await channel.fetch_message(config[message_configuration])
            await message.edit(embed=embed)
        else:
    ...
    async def on_message(self, message):
        ...
        if cparams[0] == 'ping':
            ...
            tier = 1
            while tier < 5:
                await self.report_week_tier(tier, week, newMessage)
                tier += 1
        ...
        elif cparams[0] == "reportDayGames":
            if len(cparams) > 1: await self.report_today_event(cparams[1])
            else: await self.report_today_event()
        ...
        elif cparams[0] == "setCurrentSeason":
            season = cparams[1]
            config['CurrentSeason'] = season
            save_config(config)
            await message.channel.send("Set current season to {}".format(season))
            await message.add_reaction('✔')

        elif cparams[0] == "setDailyScheduleChannel":
            if len(cparams) == 1:
                channel = message.channel.id
            else:
                channel = cparams[1]
            config['DailyScheduleChannel'] = channel
            save_config(config)
            await message.channel.send("Set daily schedule channel to {}".format(channel))
            await message.add_reaction('✔')

        elif cparams[0] == "resetTeams":
            current_teams = {0: "Unknown or undefined team"}
            current_teams = get_current_season_teams()
            config['CurrentTeams'] = current_teams
            save_config(config)
            await message.channel.send("Reset current teams")
            await message.add_reaction('✔')
