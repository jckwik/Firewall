...
class ScheduleReporter(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if cparams[0] == 'ping':
            ...
            if len(cparams) > 1:
                await self.report_today_event(cparams[1])
            else:
                await self.report_today_event()
    ...
    async def daily_refresh(self):
        ...
        tier = 1
        while tier < 5:
            await ScheduleReporter.report_week_tier(client, tier, config['CurrentWeek'], False)
            tier += 1
        await ScheduleReporter.report_today_event(client, False)
