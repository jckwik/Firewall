...
export async function execute(interaction) {
    ...
    channel = FirewallBot._client.channels.cache.get(bot.config.Tier1ResultsChannel) as TextChannel;
    channelName = "Not Found";
    if (channel) channelName = channel.name;
    embed.addField("Tier1ResultsChannel", bot.config.Tier1ResultsChannel.toString() + " (" + channelName + ")");
    channel = FirewallBot._client.channels.cache.get(bot.config.Tier2ResultsChannel) as TextChannel;
    channelName = "Not Found";
    if (channel) channelName = channel.name;
    embed.addField("Tier2ResultsChannel", bot.config.Tier2ResultsChannel.toString() + " (" + channelName + ")");
    channel = FirewallBot._client.channels.cache.get(bot.config.Tier3ResultsChannel) as TextChannel;
    channelName = "Not Found";
    if (channel) channelName = channel.name;
    embed.addField("Tier3ResultsChannel", bot.config.Tier3ResultsChannel.toString() + " (" + channelName + ")");
    channel = FirewallBot._client.channels.cache.get(bot.config.Tier4ResultsChannel) as TextChannel;
    channelName = "Not Found";
    if (channel) channelName = channel.name;
    embed.addField("Tier4ResultsChannel", bot.config.Tier4ResultsChannel.toString() + " (" + channelName + ")");
    channel = FirewallBot._client.channels.cache.get(bot.config.Tier5ResultsChannel) as TextChannel;
    channelName = "Not Found";
    if (channel) channelName = channel.name;
    embed.addField("Tier5ResultsChannel", bot.config.Tier5ResultsChannel.toString() + " (" + channelName + ")");
